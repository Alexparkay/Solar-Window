<script lang="ts">
  ...
  $: formattedEnergyRate = energyData?.averageResidentialRate ? 
    showMoney(energyData.averageResidentialRate / 100) : 'N/A'; // Convert cents to dollars
  
  // Check if the current energy cost differs from the regional average
  $: isCurrentCostDifferent = energyData?.averageResidentialRate && 
  ...
  $: percentDifference = energyData?.averageResidentialRate ? 
    Math.round(((currentEnergyCost * 100) - energyData.averageResidentialRate) / energyData.averageResidentialRate * 100) : 0;
</script>

<div class="bg-gray-800 bg-opacity-70 backdrop-blur-lg rounded-xl border border-gray-700 p-4 shadow-lg">
  ...
  {:else}
    <div class="text-gray-400 text-sm">
      No regional energy data available.
    </div>
  {/if}
</div> 